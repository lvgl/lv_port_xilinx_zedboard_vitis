 ...
 *  STATIC PROTOTYPES
 **********************/
static void profile_create(lv_obj_t * parent, lv_obj_t * mparent);
...
void lv_demo_widgets(lv_obj_t * parent)
{
    ...
    tv = lv_tabview_create(parent, LV_DIR_TOP, tab_h);
    ...
    if(disp_size == DISP_LARGE) {
        ...
        lv_label_set_text(label, "LVGL v9Dev on Zedboard");
...
static void profile_create(lv_obj_t * parent, lv_obj_t *mparent)
{
    ...
    /*Create a keyboard*/
 //   lv_obj_t * kb = lv_keyboard_create(mparent);
 //   lv_obj_add_flag(kb, LV_OBJ_FLAG_HIDDEN);
    ...
    lv_obj_add_event_cb(user_name, ta_event_cb, LV_EVENT_ALL, cpu0_globals->gui.kb_dat.kb);
    ...
    lv_obj_add_event_cb(password, ta_event_cb, LV_EVENT_ALL, cpu0_globals->gui.kb_dat.kb);
...
static void analytics_create(lv_obj_t * parent)
{
    ...
    lv_obj_set_style_bg_color(meter3, conf_p->gui_colour, LV_PART_INDICATOR);
    ...
    indic3 = lv_meter_add_needle_line(meter3, scale, 4, conf_p->gui_colour, -25);
...
//        lv_theme_default_init(NULL, lv_palette_main(*palette_primary), lv_palette_main(palette_secondary),
 //                             conf_p->gui_theme, font_normal);
        conf_p->gui_colour = lv_palette_main(*palette_primary);
        q_sys_action( &msg );
    ...
    if(code == LV_EVENT_FOCUSED) {
        if(lv_indev_get_type(lv_indev_get_act()) == LV_INDEV_TYPE_POINTER) {
            if(calendar == NULL) {
                ...
                lv_obj_add_event_cb(calendar, calendar_event_cb, LV_EVENT_VALUE_CHANGED, ta);

                lv_calendar_header_dropdown_create(calendar);
            } else lv_obj_clear_flag(calendar, LV_OBJ_FLAG_HIDDEN);
        }
    }
}
...
static void meter3_anim_cb(void * var, int32_t v)
{
	static lv_color_t last_colour;

	if( memcmp( &last_colour, &conf_p->gui_colour, sizeof(lv_color_t) ) ) {
		indic3->type_data.needle_line.color = conf_p->gui_colour;
		lv_obj_set_style_bg_color(meter3, conf_p->gui_colour, LV_PART_INDICATOR);
		last_colour = conf_p->gui_colour;
	}
